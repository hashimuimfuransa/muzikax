// Test script for beat upload validation with WhatsApp contact requirement
...
async function testBeatUploadValidation() {
  console.log('Testing beat upload validation...\n');
  ...
  try {
      ...
      const response = await axios.post(`${BASE_URL}/api/upload/track`,
        {
          ...
          genre: 'afrobeat',
          type: 'beat',
          audioURL: 'https://example.com/test-beat2.mp3',
          coverURL: 'https://example.com/test-cover2.jpg'
        },
        {
          headers: {
            'Content-Type': 'application/json',
    ...
    // Test 4: Try to upload a song without WhatsApp contact (should work)
    console.log('\nTest 4: Attempting to upload a song without WhatsApp contact');
    try {
      // First remove WhatsApp contact
      ...
      console.log('   Successfully removed WhatsApp contact');
      
      const response2 = await axios.post(`${BASE_URL}/api/upload/track`,
        {
          title: 'Test Song',
          description: 'Test song without WhatsApp contact',
          genre: 'afrobeat',
          type: 'song',
          audioURL: 'https://example.com/test-song.mp3',
          coverURL: 'https://example.com/test-cover3.jpg'
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      console.log('   ✅ Successfully uploaded song without WhatsApp contact');
      console.log('   Track ID:', response2.data._id);
      console.log('   Track title:', response2.data.title);
      console.log('   Track type:', response2.data.type);
    } catch (error) {
      console.log('   ❌ Failed to upload song without WhatsApp contact:', error.message);
      if (error.response) {
        console.log('   Response status:', error.response.status);
        console.log('   Response data:', error.response.data);
      }
