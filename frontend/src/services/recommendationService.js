    ...
    try {
        ...
        if (accessToken) {
            try {
                ...
                const response = await makeAuthenticatedRequest(`${process.env.NEXT_PUBLIC_API_URL}/api/recommendations/personalized?${personalizedParams.toString()}`);
                if (response.ok) {
                    const data = await response.json();
                    return data.tracks || [];
                }
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/recommendations/general?${fallbackParams.toString()}`);
        if (!response.ok) {
