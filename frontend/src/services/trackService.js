 ...
 */
export const fetchCreatorWhatsapp = async (creatorId) => {
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/public/creators/${creatorId}`);
        if (!response.ok) {
...
export const fetchTracksByCreatorPublic = async (creatorId) => {
    try {
        ...
        return tracks.map((track) => {
            var _a, _b;
            return (Object.assign(Object.assign({}, track), { audioUrl: track.audioURL || '', coverArt: track.coverURL || '', artist: ((_a = track.creatorId) === null || _a === void 0 ? void 0 : _a.name) || 'Unknown Artist', duration: track.duration || 0, type: track.type || 'song', creatorWhatsapp: ((_b = track.creatorId) === null || _b === void 0 ? void 0 : _b.whatsappContact) ||
 ...
 */
export const fetchTracksByType = async (type, limit = 10) => {
    try {
...
export const fetchPopularCreators = async (limit = 10) => {
    try {
 ...
 */
export const fetchCommentsForTrack = async (trackId) => {
    try {
 ...
 */
export const addCommentToTrack = async (trackId, text) => {
    try {
 ...
 */
export const checkFollowStatus = async (creatorId) => {
    try {
        let accessToken = localStorage.getItem('accessToken');
        if (!accessToken) {
