 ...
 */
const getTracksByLocation = (tracks, location) => {
    return tracks.filter(track => track.location &&
        track.location.toLowerCase().includes(location.toLowerCase()));
};
/**
 * Create premade playlists based on genres
 */
export const createGenreBasedPlaylists = async (tracks) => {
 ...
 */
export const createLocationBasedPlaylists = async (tracks, userLocation) => {
    const playlists = [];
 ...
 */
export const createMoodBasedPlaylists = (tracks) => {
    const playlists = [];
 ...
 */
export const createDecadeBasedPlaylists = (tracks) => {
    const playlists = [];
 ...
 */
export const getAllPremadePlaylists = async (tracks) => {
    ...
    const uniquePlaylists = allPlaylists.filter((playlist, index, self) => index === self.findIndex(p => p.id === playlist.id));
    return uniquePlaylists;
};
/**
 * Get featured playlists (a selection of the best premade playlists)
 */
export const getFeaturedPlaylists = async (tracks) => {
 ...
 */
export const savePremadePlaylistToUser = async (playlistId, playlistName, tracks) => {
    try {
 ...
 */
export const getUserPremadePlaylists = async () => {
    try {
        const accessToken = localStorage.getItem('accessToken');
        if (!accessToken) {
            throw new Error('User not authenticated');
