  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-gray-900 border-t border-gray-700 shadow-2xl mobile-navbar-container" style={{ paddingBottom: 'env(safe-area-inset-bottom)', zIndex: 9999 }} data-testid="mobile-navbar">
        ...
        <div className="flex justify-around">
          {navItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className={`flex flex-col items-center py-3 px-3 active:scale-95 transition-transform duration-200 ${
                isActive(item.href)
                  ? 'text-[#FF4D67] font-bold'
                  : 'text-gray-300 hover:text-white'
              }`}
            >
              <div className={isActive(item.href) ? 'text-[#FF4D67]' : 'text-gray-300'}>
                {item.icon}
              </div>
      ...
      {!currentTrack && (
        <Link 
          href="/player"
          className="absolute -top-6 left-1/2 transform -translate-x-1/2 w-12 h-12 rounded-full bg-[#FF4D67] flex items-center justify-center shadow-xl active:scale-95 transition-transform duration-200 ring-2 ring-white/20"
        >
